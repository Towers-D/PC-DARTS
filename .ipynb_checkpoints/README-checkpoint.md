## Modification
The following modifications have been made to the PC-DARTS code:

1. Changed any instance of `cuda(async=True)` to `cuda()`
2. Changed any instance of `.data[0]` to `.item()`
3. Changed line 35 in 'utils.py' from `.view(-1)` to `.reshape(-1)`
4. converted `get_lr()[0]` to `get_last_lr()[0]`
